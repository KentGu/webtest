...
FILE_TASKEXECSUMMARY=taskexec.summary_${COLLINE_VERSION}_${TIMESTRING}
FILE_TASKEXECLOG=taskexec.log_${COLLINE_VERSION}_${TIMESTRING}
...
do
 ...
 #  [ ! ${SQLFILE_} = null ] &&  sql_validation ${SQLFILE_} pre ${FILE_VAR} "${FILE_SQL_RET}" "${DB_USER}" "${DB_PASSWD}" "${DB_SERVER}" "${S_ID}"
   ...
   do
      ...
      if [ ${_TASKNAME} = "Default" ]; then
         echo ${STARTTIME_},${STARTTIME_},${TASKNAME_},Not Triggered >>${FILE_TASKEXECSUMMARY}
         if [ "${TASKFlag_}" = 1 ] && [ "${trigger_times}" -lt 3 ]; then
            ...
            # trigger_analysis             
             ...
             echo "----------break now --------" >>${FILE_TASKEXECLOG}
             break
         fi
      fi
 ...
 #        VAR_STR=`cat ${FILE_VAR} |tr -s ' ' '=' |tr '\n' ''`
         ...
         #prin
         echo ${STARTTIME_},${ENDTIME_},${TASKNAME_},$(( ${DURATION_} - 2 )),${SQL_RET_STR},${VAR_STR} >>${FILE_TASKEXECSUMMARY}  
