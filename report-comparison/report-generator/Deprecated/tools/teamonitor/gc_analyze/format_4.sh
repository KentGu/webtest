...
NOW_D=`echo ${TIMESTAMP} |cut -c 5-6`

TMPDIR=tmp_${TIMESTAMP}_$$/
OUTPUTFILE=${BASE_NAME}_OUTPUT_${TIMESTAMP}.csv
SUMMARYFILE=summary_${TIME_STRING}.csv
TMP=${TMPDIR}tmp
...
converttime(){
   ...
   START_DATE=$1
   START_TIME=$2
   RET_T=`echo $3 |awk -F. '{print$1}'`
  
   DATE_HEAD=`echo ${START_DATE} | awk -F- '{print$1,"-",$2,"-"}'`
   START_D=`echo ${START_DATE} | awk -F- '{print$3}'`
   ...
   [ ${RET_H} -gt 23 ] && RET_D=`expr ${RET_H} / 24`
   [ ${RET_H} -gt 23 ] && RET_H=`expr ${RET_H} % 24`

   START_D=`expr ${START_D} + ${RET_D}`
   RET_TS=${DATE_HEAD}${START_D},${RET_H}:${RET_M}:${RET_S} 
...
sort -n |\
   awk '{Tot+=$1}{if(NR==1){Min=$1}}{Max=$1};\
     ...
     "PF", NR, Min,Max,Tot/NR,Tot,Tot/'${TotalTime}'}\
      else{printf "%-10s %-10s %-10s %-10s %-10s %-10s %-10s\n","PF",0,0,0,0,0,0,0}}'  |tee -a summary_tmp.csv
...
cat ${OUTPUTFILE} |\
   ...
   awk '{Tot+=$1}{if(NR==1){Min=$1}}{Max=$1};\
     ...
     "CMF", NR, Min,Max,Tot/NR,Tot,Tot/'${TotalTime}'}\
      else{printf "%-10s %-10s %-10s %-10s %-10s %-10s %-10s\n","CMF",0,0,0,0,0,0,0}}' |tee -a summary_tmp.csv
...
cat ${OUTPUTFILE} |\
   ...
   awk '{Tot+=$1}{if(NR==1){Min=$1}}{Max=$1};\
     ...
     "FULLGC", NR, Min,Max,Tot/NR,Tot,Tot/'${TotalTime}'}\
      else{printf "%-10s %-10s %-10s %-10s %-10s %-10s %-10s\n","FULLGC",0,0,0,0,0,0,0}}' |tee -a summary_tmp.csv
...
cat ${OUTPUTFILE} | awk -F, '{print$9}' |\
   ...
   awk '{Tot+=$1}{if(NR==1){Min=$1}}{Max=$1};\
      ...
      else{printf "%-10s %-10s %-10s %-10s %-10s %-10s %-10s\n","TOTAL",0,0,0,0,0,0,0}}' |tee -a summary_tmp.csv
...
while read GC_LINE 
do
 echo ${GC_LINE} |awk  '{print$1,",",$2,",",$3,",",$4,",",$5,",",$6,",",$7}' >>summary_tmp2.csv
done <summary_tmp.csv

sed '1i\
EVENT,AMOUNT,MIN,MAX,AVG,TTL,PCTG OF WHOLE TIME' summary_tmp2.csv >${SUMMARYFILE}

###convert time  start
test ! ${SUMMARYONLY}x = x && test ${SUMMARYONLY} = s && rm -rf *tmp* && exit
echo `date '+%H:%M:%S'`
...
do
   ...
   cat ${FILE} | awk -F, '{print$1}' |\
      (while read X;do converttime ${STARTDATE} ${STARTTIME} ${X};done; split_signal_down)\
...
paste converttimefile ${OUTPUTFILE}|\
   sed '1 i\
   Date,Time,Seconds,NewGen,OldGen,PermGen,EntireGen,PF,CMF,FULLGC,StopTime'|\
   tr -s '\t' ' '|\
   sed 's/ //g'\
   >./../${OUTPUTFILE}
